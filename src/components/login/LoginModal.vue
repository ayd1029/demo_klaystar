<template>
  ...
  <q-dialog v-model="loginModal" persistent style="background-image: url(images/star1.jpg)">
...
export default {
  ...
  methods: {
    ...
    async sendMailCode(emailAddress) {
      ...
      this.loading = true
      const result = await this.$axios.post('/api/login/sendMailCode', vo)
      this.loading = false
    ...
    async checkEmailCode() {
      ...
      this.loading = true
      const result = await this.$axios.post('/api/login/checkEmailCode', userTempVo)
      if (result.data.resultCd === 'SUCCESS') {
        // 2. 비밀번호 암호화
    ...
    async doJoin() {
      const result = await this.$axios.post('/api/user/insertUser', this.userVo)
      this.loading = false
      // console.log(JSON.stringify(result.data))
      if (result.data.resultCd === 'SUCCESS') {
        // 성공 메세지
    ...
    async doChangePwd() {
      const result = await this.$axios.post('/api/user/updateUserPwd', this.userVo)
      this.loading = false
      // console.log(JSON.stringify(result.data))
      if (result.data.resultCd === 'SUCCESS') {
        // 성공 메세지
    ...
    doLoginKakaoStartApp () {
      ...
      const stateParam = this.$route.path + '|' + currentLocale
      
      const kakaoLoginRedirectUriCordova = this.$frontDomain + '/api/login/kakaoLoginCallbackApp'
      // const kakaoLoginRedirectUriCordova = 'http://121.162.58.101:8888' + '/api/login/kakaoLoginCallbackApp'
      try {
    ...
    async doLoginKakao (params) {
      const result = await this.$axios.post('/api/login/insertSelectUser', params)
    ...
    async doLogin () {
      ...
      // 로그인 처리
      const result = await this.$axios.post('/api/login/doLogin', this.userVo)
      if (result.data.resultCd === 'SUCCESS') { // 카카오 로그인 실패시
        // this.doLogin(result.data)
        console.log(result.data)
