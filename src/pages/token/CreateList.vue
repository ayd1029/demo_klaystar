...
export default defineComponent({
  ...
  methods: {
    ...
    goPayment(item) {
      ...
      if (item.create_status_cd === this.$CREATE_TOKEN_CD_PAID) { // 결제 완료인 경우
        // alert('결제완료 토큰')
        this.$router.push({ path: '/token/create', query: { key: item.seq }})
      } else { // 결제 화면으로 이동
        // 결제정보 설정
